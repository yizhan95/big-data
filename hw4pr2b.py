...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = np.exp(X@W)
	row_sum = mu / np.sum(mu, axis=1).reshape(-1, 1)
	log_row_sum = np.log(row_sum)
	NLL = np.sum(y*log_row_sum)+reg*np.trace(X@W)/2
...
def predict(X, W):
	...
	mu = np.exp(X@W)
	row_sum = mu / np.sum(mu, axis=1).reshape(-1, 1)
	y_pred = np.argmax(row_sum, axis=1).reshape(-1,1)
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		"*** YOUR CODE HERE ***"
		opt_w_train = grad_descent(X_train, y_train_OH, reg=reg)[0]
		pred1 = predict(X_test, opt_w_train)
		accuracy = get_accuracy(pred1, y_test)
	...
	"*** YOUR CODE HERE ***"
	index_ = np.argmax(accu_list)
	reg_opt = lambda_list[index_]
