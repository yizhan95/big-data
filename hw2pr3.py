...
def linreg(X, y, reg=0.0):
	...
	Id[0, 0] = 0
	lambdaI = reg*Id
	W_opt=np.linalg.solve(lambdaI+X.T@X, X.T@y)
...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	m, n= X.shape
	MSE=np.linalg.norm(predict(W, X) - y)**2/m
	RMSE= np.sqrt(MSE)
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg=element) for element in reg_list]
	RMSE_list = [find_RMSE(element, X_val, y_val) for element in W_list]
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg=element) for element in reg_list]
	norm_list=[np.linalg.norm(element) for element in W_list]
...
def linreg_no_bias(X, y, reg=0.0):
	...
	lambdaI2=reg*Id2
	one2=np.ones((m, m))
	W_opt=np.linalg.solve(lambdaI2+X.T@(Id3-one2/m)@X, X.T@(Id3-one2/m)@y)
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		train_rmse = np.sqrt(np.linalg.norm(X_train@W +b - y_train)**2/m_train)
		val_rmse = np.sqrt(np.linalg.norm(X_val@W +b - y_val)**2/m_val)
		obj_train.append(train_rmse)
		obj_val.append(val_rmse)
		
		
		W_grad = (X_train.T@X_train@W+X_train.T@(b- y_train)+lambdaI2@W)/m_train
		b_grad = (sum(X_train@W)+ b*m_train - sum(y_train))/m_train
		W -= lr_W * W_grad
		b -= lr_b * b_grad
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	column1 = np.ones_like(X_train)
	column2 = np.ones_like(X_val)
	column3 = np.ones_like(X_test)

	X_train = np.hstack((column1, X_train))
	X_val = np.hstack((column2, X_val))
	X_test = np.hstack((column3, X_test))
