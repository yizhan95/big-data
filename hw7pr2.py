...
def find_cost(X, y, W , reg):
	...
	m, n = X.shape
	a = X@W-y
	cost = float(a.T@a)+ reg*np.abs(W).sum()
	cost = cost/m
	"*** END YOUR CODE HERE ***"
	return cost
...
def grad_lasso(
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		ind_list = np.random.randint(0,m, size = batch_size)
		W_grad = find_grad(X[ind_list], y[ind_list], W, reg=reg)
		W = prox(W-lr*W_grad, lr*reg)
		cost = find_cost(X[ind_list], y[ind_list], W, reg=reg)
...
def lasso_path(X, y, tau_min=1e-8, tau_max=10, num_reg=10):
	...
	for index in range(num_reg):
		...
		# 		as they are.
		"""*** YOUR CODE HERE ***"""
		W[:, index] = grad_lasso(X,y,reg=reg, lr=1e-12, eps=1e-5, max_iter=300, batch_size=1024, print_freq=1)[0].flatten()
...
if __name__ == '__main__':
	...
	"""*** YOUR CODE HERE ***"""
	index = np.argsort(-W[:,0])[:5]+1
	top_features = np.array(df.columns)[index]
