"""
Starter file for nmf (hw8 extra credit) of Big Data Summer 2017
...
def nmf_cost(X, W, H):
	...
	"*** YOUR CODE HERE ***"
	sparse_matrix = X.tocoo()
	for i in range(len(sparse_matrix.data)):
		width = sparse_matrix.row[i]
		height = sparse_matrix.col[i]
		WH = W[width, :]@H[:,height]
		data = sparse_matrix.data[i]
		update_value = (data - WH.item(0))**2
		cost = cost + update_value
...
def nmf(X, k=20, max_iter=100, print_freq=5):
	...
	for iter_num in range(max_iter):
		...
		"*** YOUR CODE HERE ***"
		H = H* (W.T@X)/(W.T@W@H) #from slide lecture 9 p37
		W = W* (X@H.T) / (W@H@H.T)
		cost = nmf_cost(X, W, H)
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	max_index = np.argsort(H)
	ind = np.flip(max_index, axis=1)
