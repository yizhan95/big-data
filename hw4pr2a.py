...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu= sigmoid(X@W)
	a1 = np.multiply(y, np.log(mu))
	a2 = np.multiply((1.-y), np.log(1.-mu))
	nll = -sum(a1+a2)+ reg/2*np.linalg.norm(W)**2
...
def newton_step(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu=sigmoid(X@W)
	gradOfLog = grad_logreg(X, y, W, reg=reg)
	S = np.diag(np.squeeze(np.asarray(np.multiply(mu, (1.0-mu)))))
	H = X.T@S@X+reg*np.eye(X.shape[1])
	d = np.linalg.solve(H, gradOfLog)
...
def get_description(X, y, W):
	...
	recall_total = 0
	for i in range(m):
		actual_value = y.item(i)
		predict_value = pred.item(i)
	...
	accuracy =	1.0* accuracy_count / m
	precision = 1.0* predict_count / predict_total
	recall = 1.0* recall_count /  recall_total 
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0.01, 0.1, 0.5, 1.0, 10.0, 50.0, 100.0, 200.0, 500.0, 1000.0]
	for i in range(len(reg_list)):
		W_opt,nll_list = grad_descent(X_train, y_train, reg=reg_list[i], lr=2e-4, print_freq=100)
		accurary, precision, recall, f1 = get_description(X_test, y_test, W_opt)
		a_list.append(accurary)
	...
	"*** YOUR CODE HERE ***"
	W_opt_index = np.argmax(a_list)
	reg_opt = reg_list[W_opt_index]
