...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for index in range(m):
			pt = X[index, :]
			matr = pt - clusters
			dist = np.linalg.norm(matr, axis=1)
			shortest_index = np.argsort(dist)[0]
			label[index] = shortest_index

		
		for index_2 in range(k):
			q = np.where(label == index_2)[0]
			if len(q) >0:
				clusters[index_2, :] = np.mean(X[q], axis=0)
...
if __name__ == '__main__':
	...
	for k in range(1, 21):
		...
		"*** YOUR CODE HERE ***"
		cluster, label, cost_list = k_means(X, k)
